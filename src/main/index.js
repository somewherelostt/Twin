...
import { TOOLKITS } from "../shared/contracts";
...
function registerIpc() {
    ...
    ipcMain.handle("twin:set-mode", (_event, mode) => settings.set("mode", mode));
    ipcMain.handle("twin:connections", async () => TOOLKITS.map((slug) => ({ slug, connected: false })));
    ipcMain.handle("twin:connect", async (_event, toolkit) => {
        if (!TOOLKITS.includes(toolkit))
            throw new Error("Unsupported integration");
        if (!process.env.COMPOSIO_API_KEY)
            throw new Error("Add COMPOSIO_API_KEY to connect apps");
        throw new Error("Connection service is not initialized yet");
    });
    ipcMain.handle("twin:open-external", async (_event, url) => {
        const parsed = new URL(url);
        if (parsed.protocol !== "https:")
            throw new Error("Only secure links are allowed");
